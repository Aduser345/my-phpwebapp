...
print_r($_POST);
print_r($_SESSION);
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && (!isset($_GET['delanswer']))) {
    $eid   = @$_GET['eid'];
    $sn    = @$_GET['n'];
    $total = @$_GET['t'];
    $ans   = $_POST['ans'];
    ...
    // History that Quiz has been started yet or not
    ...
    if (mysqli_num_rows($q) > 0) {
        ...
        // If the quiz is enabled and ongoing
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {

            // Get User Answer 
            $query1 = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND username='$_SESSION[db_user_name]' AND qid='$qid' ") or die('Error115');
            
            // Save old answer if any
            while ($row = mysqli_fetch_array($q)) {
                $prevans = $row['ans'];
            }

            // Get original answer of the question
            $q = mysqli_query($con, "SELECT * FROM answer WHERE qid='$qid' ");
            while ($row = mysqli_fetch_array($q)) {
                // $ansid tells us the correct answer
                $ansid = $row['ansid'];
            }

            // Again get
            // $q = mysqli_query($con, "SELECT * FROM user_answer WHERE username='$_SESSION[db_user_name]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error1977');
            if (mysqli_num_rows($query1) != 0) {
                // Update user answer 
                $q = mysqli_query($con, "UPDATE user_answer SET ans='$ans' WHERE username='$_SESSION[db_user_name]' AND eid='$_GET[eid]' AND qid='$qid' ") or die('Error197');
            } 
            else {
                // Insert user answer if not already present
                $q = mysqli_query($con, "INSERT INTO user_answer VALUES(NULL,'$qid','$ans','$ansid','$_GET[eid]','$_SESSION[db_user_name]')");
            }
            ...
            // Checking if the answer selected and the correct one
            ...
            if ($ans == $ansid) {
                ...
                // If answer is correct...
                // Fetch correct and wrong ke marks (Scheme of question)
                ...
                // Retrieving old saved data from history
                ...
                if (isset($prevans) && $prevans == $ansid) {
                    ...
                    // chosen answer is not equal to the prev saved answer
                    $r++;
                    $w--;

                    // adding one correct answer and removing the old incorrect answer
                    $s = $s + $correct + $wrong;
                    ...
                    // If there is no old answer saved right answer++
                    $r++;
                    $s = $s + $correct;
            ...
            $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='db_user_name]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[db_user_name]'") or die('Error156');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $scorestatus = $row['score_updated'];
                }
                if($scorestatus=="false"){
                    $q = mysqli_query($con, "UPDATE history SET score_updated='true' WHERE username='$_SESSION[db_user_name]' AND eid='$_GET[eid]' ") or die('Error197');
        ...
        // echo "DADADAD";
        unset($_SESSION['6e447159425d2d']);
        $q = mysqli_query($con, "UPDATE history SET status='finished' WHERE username='$_SESSION[db_user_name]' AND eid='$_GET[eid]' ") or die('Error197');
        $q = mysqli_query($con, "SELECT * FROM history WHERE eid='$_GET[eid]' AND username='$_SESSION[db_user_name]'") or die('Error156');
                while ($row = mysqli_fetch_array($q)) {
                    $s = $row['score'];
                    $scorestatus = $row['score_updated'];
...
if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2 && isset($_SESSION['6e447159425d2d']) && $_SESSION['6e447159425d2d'] == "6e447159425d2d" && (!isset($_POST['ans'])) && (isset($_GET['delanswer'])) && $_GET['delanswer'] == "delanswer") {
    ...
    $q = mysqli_query($con, "SELECT * FROM history WHERE username='$username' AND eid='$_GET[eid]' ") or die('Error197');
    if (mysqli_num_rows($q) > 0) {
        $q = mysqli_query($con, "SELECT * FROM history WHERE username='$_SESSION[db_user_name]' AND eid='$_GET[eid]' ") or die('Error197');
        while ($row = mysqli_fetch_array($q)) {
            $time   = $row['timestamp'];
            $status = $row['status'];
        }
        ...
        if ($status == "ongoing" && $remaining > 0 && $qstatus == "enabled") {
            ...
            $q = mysqli_query($con, "SELECT * FROM user_answer WHERE eid='$_GET[eid]' AND username='$_SESSION[db_user_name]' AND qid='$qid' ") or die('Error115');
            $row = mysqli_fetch_array($q);
            $ans = $row['ans'];
            $q = mysqli_query($con, "DELETE FROM user_answer WHERE qid='$qid' AND username='$_SESSION[db_user_name]' AND eid='$_GET[eid]' ") or die("Error2222");
            if ($ans == $ansid) {
                $q = mysqli_query($con, "SELECT * FROM quiz WHERE eid='$eid' ") or die('Error129');
                while ($row = mysqli_fetch_array($q)) {
                    $wrong   = $row['wrong'];
